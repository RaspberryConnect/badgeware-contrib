...
def load_config():
    global dicetype1
    global dicetype2
    config = {"dice1":dicetype1,"dice2":dicetype2}
    if State.load("bw_dicetype",config) == 1:
        dicetype1 = config["dice1"]
        dicetype2 = config["dice2"]
    else:
        save_config()
...
def change_out(pos):
    ...
    twx = [tween(dots[i][0]+offset,fin,300,tween.QUAD_IN).start() for i in range(len(dots))]
    while not twx[-1].done:
        scrn_message()
        place_dots(throwSW,swap)
        for j in range(len(twx)):
            screen.shape(shape.circle(twx[j].now,dots[j][1],8))
        badge.update()
...
def menu():
    ...
    while 1:
        ...
        screen.text(str(dicetype1),30,40,40)
        screen.text(str(dicetype2),100,40,40)
        i1 = option.index(dicetype1)
        i2 = option.index(dicetype2)
...
while True:
    if badge.pressed(BUTTON_A):
        change_out(1)
        throw1 = roll(dicetype1)
        fade_in(throw1,1)
        d=1
    if badge.pressed(BUTTON_C):
        change_out(2)
        throw2 = roll(dicetype2)
        fade_in(throw2,2)
        d=1
    if badge.pressed(BUTTON_B):
        change_out(1)
